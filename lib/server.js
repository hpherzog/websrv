...
function Server (options) {
    ...
    this.sslKeyPath = options.sslKeyPath !== void(0)?
        options.sslKeyPath : path.normalize(this.basePath + '/ssl/key.pem');

    this.sslCertPath = options.sslCertPath !== void(0)?
        options.sslCertPath : path.normalize(this.basePath + '/ssl/crt.pem');
    ...
    this.viewPath = _.isString(options.viewPath)?
...
Server.prototype.createExpressApp = function() {
    ...
    if(!_.isString(this.cookieSecret)) {
        self.logger.warn('You should set property cookieSecret!');
    }
...
Server.prototype.getStartHandler = function getStartHandler() {
    ...
    return function() {
        ...
        self.logger.info('SSLKey: ' + self.sslKeyPath);
        self.logger.info('SSLCert: ' + self.sslCertPath);
